...
<body onload="startTime()">
	...
	<div class="menu-bar"
		...
		<div class="title-content">Detail user</div>
		<form
			action="${pageContext.request.contextPath}/search_disputes_iss_card"
			method="post" style="position: fixed; margin-top: 75px;">
			<c:forEach items="${detailUser}" var="p">
				<table class="form-search-iss"
					style="width: 100%; margin-left: -5%;">
					<tr>
						<td><span>USER ID</span> ${p.username }</td>
						<td><span>FULL NAME</span> ${p.fullname }</td>
						<td><span>First login date(yyyymmdd) </span> <input
							name="userFirstTimeLogin" class="form-control"
							value="${p.userFirstTimeLogin}" /></td>
						<td><span>Expiry Date(dd/mm/yyyy) (yyyymmdd) </span> <input
							name="userFirstTimeLogin" class="form-control"
							value="202012131961817988" /></td>
					</tr>
					<tr>
						<td><span>Attempted</span> <input name="userLoginFail"
							class="form-control" value="${p.userLoginFail}" /></td>
						<td><span>BLOCKS?</span> <c:choose>
								<c:when test="${p.status == '1'}">
									<input type="checkbox" name="user_status" class="form-control"
										value="1" checked />
								</c:when>
								<c:otherwise>
									<input type="checkbox" t name="user_status"
										class="form-control" />
								</c:otherwise>
							</c:choose></td>
						<td><input type="submit" value="UPDATE"
							style="float: left; width: 80%; visibility: hidden;"
