...
<body onload="startTime()">
	...
	<div class="menu-bar"
		...
		<div class="title-content">Truy vấn/Cập nhật ACQ_POS_VTB</div>
		<form
			action="${pageContext.request.contextPath}/dmt1/search_acq_pos_vtb"
			...
			<table class="form-search-iss" style="width: 100%; margin-left: -5%;">
				<tr>
					...
					<td><span>MID</span> <c:choose>
							<c:when test="${acqPosVtbMid != '' && acqPosVtbMid != 'null'}">
								<input name="acqPosVtbMid" class="form-control"
									value="${acqPosVtbMid}" />
							</c:when>
							<c:otherwise>
								<input name="acqPosVtbMid" class="form-control" />
					...
					<td><span>TID</span> <c:choose>
							<c:when test="${acqPosVtbTid != '' && acqPosVtbTid != null}">
								<input name="acqPosVtbTid" class="form-control"
									value="${acqPosVtbTid}" />
							</c:when>
							<c:otherwise>
								<input name="acqPosVtbTid" class="form-control" value="" />
							</c:otherwise>
						</c:choose></td>
					<td><span>Loại thẻ</span> <select style="width: 85%"
						class="form-control" id="loaiThe" name="acqPosVtb_loaiThe">
				...
				<tr>
					<td><span>Số thẻ đầu</span> <c:choose>
							<c:when
								test="${acqPosVtbsoTheDau != '' && acqPosVtbsoTheDau != 'null'}">
								<input name="acqPosVtbsoTheDau" class="form-control"
									value="${acqPosVtbsoTheDau}" />
							</c:when>
							<c:otherwise>
								<input name="acqPosVtbsoTheDau" class="form-control" />
							</c:otherwise>
					...
					<td><span>Số thẻ cuối</span> <c:choose>
							<c:when
								test="${acqPosVtbsoTheCuoi != '' && acqPosVtbsoTheCuoi != null}">
								<input name="acqPosVtbsoTheCuoi" class="form-control"
									value="${acqPosVtbsoTheCuoi}" />
							</c:when>
							<c:otherwise>
								<input name="acqPosVtbsoTheCuoi" class="form-control" value="" />
							</c:otherwise>
						</c:choose></td>
				...
				<table rules="groups" frame="hsides" id="testTable"
					...
					<thead valign="top">
						<tr class="header-table">
							...
							<th style="width: 10%">Địa chỉ GD</th>
							...
							<th style="width: 10%">Số tiền TIP</th>
							<th style="width: 10%">Số thẻ</th>
							<th style="width: 10%">Loại thẻ</th>
							<th style="width: 10%">Kết quả GD</th>
							<th style="width: 10%">Đảo/hủy</th>
					...
					<tbody>
						<c:forEach items="${listAcqPosVtb}" var="p">
							<tr id="rows" class="rows">
								<td style="width: 10%;">${p.acqPosVtbMid}</td>
								<td style="width: 10%;">${p.acqPosVtbMerchantName}</td>
								<td style="width: 10%;">${p.acqPosVtbTid}</td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;">${p.acqPosVtbtrxnDate}</td>
								<td style="width: 10%;">${p.acqPosVtbApproveCode}</td>
								<td style="width: 10%;">${p.acqPosVtbTxnAmt}</td>
								<td style="width: 10%;">${p.acqPosVtb_LoaiTien}</td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;">${p.acqPosVtbsoTheDau}XXXXXX
									${p.acqPosVtbsoTheCuoi}</td>
								<td style="width: 10%;">${p.acqPosVtbloaiThe}</td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;">${p.acqPosVtbPosMode}</td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;">${p.acqPosVtbtamGiuBaoCo}</td>
								<td style="width: 10%;">${p.acqPosVtbBranch}</td>
								<td style="width: 10%;">${p.acqPosVtbCanBoPhuTrach}</td>
								<td style="width: 10%;"></td>
								<td style="width: 10%;">${p.acqPosVtbtienDoXuLy}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
